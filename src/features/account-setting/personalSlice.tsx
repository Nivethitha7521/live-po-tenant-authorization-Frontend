...
export const fetchPersonals = createAsyncThunk('personals/fetchPersonals', async () => {
  const response = await axios.get('http://192.168.29.117:8000/purchaseapi/popersonals/'); // Adjust API endpoint as needed
...
export const addPersonal = createAsyncThunk<Personal, Personal>('personals/addPersonal', async (personalData) => {
  const response = await axios.post('http://192.168.29.117:8000/purchaseapi/popersonals/', personalData); // Adjust API endpoint as needed
  return response.data;
});
...
export const updatePersonal = createAsyncThunk<Personal, Personal>('personals/updatePersonal', async (personalData) => {
  const response = await axios.patch(`http://192.168.29.117:8000/purchaseapi/popersonals/${personalData.personalId}`, personalData); // Adjust API endpoint as needed
  return response.data;
});
...
const personalSlice = createSlice({
  ...
  reducers: {
    ...
    updatePersonaldetail: (state, action) => {
      const index = state.personalitems.findIndex(person => person.personalId === action.payload.personalId);
      if (index !== -1) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updatePersonal.fulfilled, (state, action) => {
        const index = state.personalitems.findIndex((personalitems) => personalitems.personalId === action.payload.personalId);
        if (index !== -1) {
...
export const {
  ...
  setSnackbarOpen,
  setSnackbarMessage,
  setEditIndex,addPersonaldetail,updatePersonaldetail
