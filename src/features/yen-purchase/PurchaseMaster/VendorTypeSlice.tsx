...
export const fetchVendorTypeItems = createAsyncThunk<VendorTypeItem[], { signal?: AbortSignal }>(
  ...
  async ({ signal }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get('http://192.168.29.117:8000/purchaseapi/vendortypes/', {
...
export const addVendorTypeItem = createAsyncThunk<VendorTypeItem, { data: VendorTypeItem; signal?: AbortSignal }>(
  ...
  async ({ data, signal }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post('http://192.168.29.117:8000/purchaseapi/vendortypes', data, {
        cancelToken: source.token,
      });
      return response.data;
    } catch (error: any) {
...
>(
  ...
  async ({ vendortypeId, vendortype, signal }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.patch(`http://192.168.29.117:8000/purchaseapi/vendortypes/${vendortypeId}`, vendortype, {
        cancelToken: source.token,
      });
      return response.data;
    } catch (error: any) {
  ...
  async ({ vendortypeId, signal }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.patch(
        `http://192.168.29.117:8000/purchaseapi/vendortypes/${vendortypeId}`,
        { status: 'deactivated' },
        { cancelToken: source.token }
      );
      return response.data;
