...
export const fetchCategories = createAsyncThunk<Category[]>('category/fetchCategories', async () => {
  const response = await axios.get('https://yenerp.com/purchasetestapi/purchasecategories/');
  return response.data;
});

export const addCategory = createAsyncThunk<Category, Category>('category/addCategory', async (category) => {
  const response = await axios.post('https://yenerp.com/purchasetestapi/purchasecategories', category);
  return response.data;
});
  ...
  async ({ categoryId, subcategory }, { rejectWithValue }) => {
    try {
      const response = await axios.patch(
        `https://yenerp.com/purchasetestapi/purchasecategories/${categoryId}/subcategories/remove`,
...
export const updateCategory = createAsyncThunk<Category, { purchasecategoryId: string, category: Category }>(
  ...
  async ({ purchasecategoryId, category }) => {
    const response = await axios.patch(`https://yenerp.com/purchasetestapi/purchasecategories/${purchasecategoryId}`, category);
    return response.data;
  }
);

export const deactivateCategory = createAsyncThunk<Category, string>('category/deactivateCategory', async (purchasecategoryId) => {
  const response = await axios.patch(`https://yenerp.com/purchasetestapi/purchasecategories/${purchasecategoryId}`, { status: 'deactivated' });
  return response.data;
});

export const activateCategory = createAsyncThunk<Category, string>('category/activateCategory', async (purchasecategoryId) => {
  const response = await axios.patch(`https://yenerp.com/purchasetestapi/purchasecategories/${purchasecategoryId}`, { status: 'active' });
  return response.data;
});
  ...
  async (file: File, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post(
        'https://yenerp.com/purchasetestapi/purchasecategories/import_csv',
        formData,
        {
          headers: {
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get(
        'https://yenerp.com/purchasetestapi/purchasecategories/exportcategory/export_csv',
        {
          responseType: 'blob',
        }
      );
