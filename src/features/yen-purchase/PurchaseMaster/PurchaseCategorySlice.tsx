...
export const fetchCategories = createAsyncThunk<Category[]>('category/fetchCategories', async () => {
  const response = await axios.get('http://192.168.29.116:8000/purchaseapi/purchasecategories/');
  return response.data;
});

export const addCategory = createAsyncThunk<Category, Category>('category/addCategory', async (category) => {
  const response = await axios.post('http://192.168.29.116:8000/purchaseapi/purchasecategories', category);
  return response.data;
});
  ...
  async ({ categoryId, subcategory }, { rejectWithValue }) => {
    try {
      const response = await axios.patch(
        `http://192.168.29.116:8000/purchaseapi/purchasecategories/${categoryId}/subcategories/remove`,
...
export const updateCategory = createAsyncThunk<Category, { purchasecategoryId: string, category: Category }>(
  ...
  async ({ purchasecategoryId, category }) => {
    const response = await axios.patch(`http://192.168.29.116:8000/purchaseapi/purchasecategories/${purchasecategoryId}`, category);
    return response.data;
  }
);

export const deactivateCategory = createAsyncThunk<Category, string>('category/deactivateCategory', async (purchasecategoryId) => {
  const response = await axios.patch(`http://192.168.29.116:8000/purchaseapi/purchasecategories/${purchasecategoryId}`, { status: 'deactivated' });
  return response.data;
});

export const activateCategory = createAsyncThunk<Category, string>('category/activateCategory', async (purchasecategoryId) => {
  const response = await axios.patch(`http://192.168.29.116:8000/purchaseapi/purchasecategories/${purchasecategoryId}`, { status: 'active' });
  return response.data;
});

export const fetchSubcategories = createAsyncThunk<Subcategory[]>('subcategory/fetchSubcategories', async () => {
  const response = await axios.get('http://192.168.29.116:8000/purchaseapi/purchasesubcategories/');
  return response.data;
});
  ...
  async (file: File, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post(
        'http://192.168.29.116:8000/purchaseapi/purchasecategories/import_csv',
        formData,
        {
          headers: {
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get(
        'http://192.168.29.116:8000/purchaseapi/purchasecategories/exportcategory/export_csv',
        {
          responseType: 'blob',
        }
      );
