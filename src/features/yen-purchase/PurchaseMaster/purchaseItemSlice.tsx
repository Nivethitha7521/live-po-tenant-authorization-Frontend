...
const EXPORT_CSV_URL = 'http://192.168.29.116:8000/purchaseapi/rawMaterials/export_csv'; // Make sure this URL is correct
...
export const fetchPurchaseItems = createAsyncThunk(
  ...
  async ({
    ...
    try {
      const response = await axios.get('http://192.168.29.116:8000/purchaseapi/rawMaterials/', { params });
      return {
...
export const fetchPurchaseCategories = createAsyncThunk('purchaseSubcategory/fetch', async () => {
  const response = await axios.get<PurchaseCategory[]>('http://192.168.29.116:8000/purchaseapi/purchasecategories/');
  return response.data;
});

// Fetch all purchase uoms
export const fetchUom = createAsyncThunk('uom/fetch', async () => {
  const response = await axios.get<UOM[]>('http://192.168.29.116:8000/purchaseapi/purchaseuoms/');
...
export const fetchPurchaseTaxes = createAsyncThunk('purchaseTaxes/fetch', async () => {
  const response = await axios.get<PurchaseTax[]>('http://192.168.29.116:8000/purchaseapi/purchasetaxes/');
...
export const fetchStorageLocationItems = createAsyncThunk('storageLocations/fetch', async () => {
  const response = await axios.get<StorageLocationItem[]>('http://192.168.29.116:8000/purchaseapi/storagelocations/');
  ...
  async (purchase: Omit<PurchaseItem, 'purchaseitemId'>, { dispatch }) => {
    try {
      ...
      const response = await axios.post<PurchaseItem>(
        'http://192.168.29.116:8000/purchaseapi/rawMaterials/', // Ensure the endpoint matches your backend route
        purchaseToAdd
      );
...
export const POsearchPurchaseItems = createAsyncThunk<PurchaseItemSearch[], { searchQuery: string, skip: number, limit: number }>(
  ...
  async ({ searchQuery, skip, limit }) => {
    ...
    const response = await axios.get<PurchaseItemSearch[]>(`http://192.168.29.116:8000/purchaseapi/rawMaterials/exact-name/`, {
      params: {
...
export const searchPurchaseItems = createAsyncThunk<PurchaseItemSearchAdd[], { searchQuery: string; skip: number; limit: number; forceRefresh?: boolean }>(
  ...
  async ({ searchQuery, skip, limit, forceRefresh = false }) => {
    ...
    try {
      // Fetch fresh data from the API if no valid cache is found
      const response = await axios.get<SearchResponse>(`http://192.168.29.116:8000/purchaseapi/rawMaterials/search`, {
  ...
  async (purchase: PurchaseItem, { dispatch }) => {
    try {
      ...
      const response = await axios.patch<PurchaseItem>(
        `http://192.168.29.116:8000/purchaseapi/rawMaterials/${purchase.purchaseitemId}`,
        purchaseToUpdate
      );
...
export const deactivatePurchaseItem = createAsyncThunk(
  ...
  async (id: string) => {
    const response = await axios.patch<PurchaseItem>(
      `http://192.168.29.116:8000/purchaseapi/rawMaterials/${id}`,
      { status: 'deactivated' }
    );
  ...
  async ({ file, mode }: ImportPayload, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post(
        'http://192.168.29.116:8000/purchaseapi/rawMaterials/import_csv', 
        formData, 
        {
          headers: {
...
export const exportPurchaseItems = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get('http://192.168.29.116:8000/purchaseapi/rawMaterials/purchaseitemexport/export_csv', {
        responseType: 'blob',
      });
