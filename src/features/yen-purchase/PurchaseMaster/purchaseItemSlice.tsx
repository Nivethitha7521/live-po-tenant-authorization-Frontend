...
const EXPORT_CSV_URL = 'https://yenerp.com/purchasetestapi/rawMaterials/export_csv'; // Make sure this URL is correct
...
export const fetchPurchaseItems = createAsyncThunk(
  ...
  async ({
    ...
    try {
      const response = await axios.get('https://yenerp.com/purchasetestapi/rawMaterials/', { params });
      return {
...
export const fetchPurchaseCategories = createAsyncThunk('purchaseSubcategory/fetch', async () => {
  const response = await axios.get<PurchaseCategory[]>('https://yenerp.com/purchasetestapi/purchasecategories/');
  return response.data;
});

// Fetch all purchase uoms
export const fetchUom = createAsyncThunk('uom/fetch', async () => {
  const response = await axios.get<UOM[]>('https://yenerp.com/purchasetestapi/purchaseuoms/');
...
export const fetchPurchaseTaxes = createAsyncThunk('purchaseTaxes/fetch', async () => {
  const response = await axios.get<PurchaseTax[]>('https://yenerp.com/purchasetestapi/purchasetaxes/');
...
export const fetchStorageLocationItems = createAsyncThunk('storageLocations/fetch', async () => {
  const response = await axios.get<StorageLocationItem[]>('https://yenerp.com/purchasetestapi/storagelocations/');
...
export const fetchPurchaseGroupItems = createAsyncThunk('groupItems/fetch', async () => {
  const response = await axios.get<PurchaseGroupItem[]>('https://yenerp.com/purchasetestapi/itemgroups/');
  ...
  async (purchase: Omit<PurchaseItem, 'purchaseitemId'>, { dispatch }) => {
    try {
      ...
      const response = await axios.post<PurchaseItem>(
        'https://yenerp.com/purchasetestapi/rawMaterials/', // Ensure the endpoint matches your backend route
        purchaseToAdd
      );
...
export const POsearchPurchaseItems = createAsyncThunk<PurchaseItemSearch[], { searchQuery: string, skip: number, limit: number }>(
  ...
  async ({ searchQuery, skip, limit }) => {
    ...
    const response = await axios.get<PurchaseItemSearch[]>(`https://yenerp.com/purchasetestapi/rawMaterials/exact-name/`, {
      params: {
...
export const searchPurchaseItems = createAsyncThunk<PurchaseItemSearchAdd[], { searchQuery: string; skip: number; limit: number; forceRefresh?: boolean }>(
  ...
  async ({ searchQuery, skip, limit, forceRefresh = false }) => {
    ...
    try {
      // Fetch fresh data from the API if no valid cache is found
      const response = await axios.get<SearchResponse>(`https://yenerp.com/purchasetestapi/rawMaterials/search`, {
...
export const deactivatePurchaseItem = createAsyncThunk(
  ...
  async (id: string) => {
    const response = await axios.patch<PurchaseItem>(
      `https://yenerp.com/purchasetestapi/rawMaterials/${id}`,
      { status: 'deactivated' }
    );
...
export const exportPurchaseItems = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get('https://yenerp.com/purchasetestapi/rawMaterials/purchaseitemexport/export_csv', {
        responseType: 'blob',
      });
