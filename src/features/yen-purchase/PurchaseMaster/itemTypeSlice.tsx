...
export const addPurchaseTypeItem = createAsyncThunk<PurchaseItemType, Omit<PurchaseItemType, 'itemtypeId'>>('purchaseTypeItems/addPurchaseTypeItem', async (groupItemData, { rejectWithValue }) => {
  try {
    const response = await axios.post('http://192.168.29.117:8000/purchaseapi/itemtypes/', groupItemData);
    return response.data;
  } catch (error: any) {
...
export const updatePurchaseTypeItem = createAsyncThunk<PurchaseItemType, PurchaseItemType>('purchaseTypeItems/updatePurchaseTypeItem', async (groupItemData, { rejectWithValue }) => {
  try {
    const response = await axios.patch(`http://192.168.29.117:8000/purchaseapi/itemtypes/${groupItemData.itemtypeId}`, groupItemData);
    return response.data;
  } catch (error: any) {
    return rejectWithValue(error.response?.data?.detail || error.message);
  }
...
export const deactivatePurchaseTypeItem = createAsyncThunk<PurchaseItemType, string>('purchaseTypeItems/deactivatePurchaseTypeItem', async (itemtypeId, { rejectWithValue }) => {
  try {
    const response = await axios.patch(`http://192.168.29.117:8000/purchaseapi/itemtypes/${itemtypeId}`, { status: 'deactivated' });
    return response.data;
  } catch (error: any) {
    return rejectWithValue(error.response?.data?.detail || error.message);
  }
...
export const activatePurchaseTypeItem = createAsyncThunk<PurchaseItemType, string>('purchaseTypeItems/activatePurchaseTypeItem', async (itemtypeId, { rejectWithValue }) => {
  try {
    const response = await axios.patch(`http://192.168.29.117:8000/purchaseapi/itemtypes/${itemtypeId}`, { status: 'active' });
    return response.data;
  } catch (error: any) {
    return rejectWithValue(error.response?.data?.detail || error.message);
  }
  ...
  async (file: File, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post(
        'http://192.168.29.117:8000/purchaseapi/itemtypes/import-csv',
        formData,
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get(
        'http://192.168.29.117:8000/purchaseapi/itemtypes/export-itemtype/export-csv',
        { responseType: 'blob' }
      );
