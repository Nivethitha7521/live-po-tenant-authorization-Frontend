...
export const fetchVendorAll = createAsyncThunk(
  ...
  async ({
    ...
    try {
      const response = await axios.get('http://192.168.29.116:8000/purchaseapi/vendors/limit', { params });
...
export const fetchVendorNames = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get('http://192.168.29.116:8000/purchaseapi/vendors/vendorname');
...
export const addVendor = createAsyncThunk(
  ...
  async (vendor: Omit<Vendor, 'vendorId' | 'createdDate' | 'updatedDate'>) => {
    try {
      ...
      const response = await axios.post<Vendor>('http://192.168.29.116:8000/purchaseapi/vendors', vendorToAdd);
...
>(
  ...
  async ({ vendor_name, skip, limit, forceRefresh = false }) => {
    ...
    const response = await axios.get<VendorSearch[]>(`http://192.168.29.116:8000/purchaseapi/vendors/exact-name/`, {
...
export const searchVendors = createAsyncThunk<VendorSummary[], { searchQuery: string; skip: number; limit: number; forceRefresh?: boolean }>(
  ...
  async ({ searchQuery, skip, limit, forceRefresh = false }) => {
    ...
    const response = await axios.get<VendorSummary[]>(`http://192.168.29.116:8000/purchaseapi/vendors/vendor-names/`, {
    ...
    try {
      ...
      try {
        const response = await axios.patch<Vendor>(
          `http://192.168.29.116:8000/purchaseapi/vendors/${vendorId}`,
          vendorToUpdate,
          {
            headers: {
...
export const deactivateVendor = createAsyncThunk('vendors/deactivate', async (vendorId: string) => {
  try {
    await axios.patch<Vendor>(`http://192.168.29.116:8000/purchaseapi/vendors/${vendorId}`, { status: 'deactivated' });
    return vendorId;
  } catch (error: any) {
...
export const activateVendor = createAsyncThunk('vendors/activate', async (vendorId: string) => {
  try {
    const response = await axios.patch<Vendor>(`http://192.168.29.116:8000/purchaseapi/vendors/${vendorId}`, { status: 'active' });
    return vendorId;
  } catch (error: any) {
  ...
  async (file: File, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post<CsvImportResponse>(
        'http://192.168.29.116:8000/purchaseapi/vendors/import-csv',
        formData,
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get<Blob>(
        'http://192.168.29.116:8000/purchaseapi/vendors/exportvendor/export-csv',
        {
...
export const fetchVendorTypeItems = createAsyncThunk('vendorTypes/fetch', async (_, { rejectWithValue }) => {
  try {
    const response = await axios.get<VendorTypeItem[]>('http://192.168.29.116:8000/purchaseapi/vendortypes/');
    return response.data;
  } catch (error: any) {
