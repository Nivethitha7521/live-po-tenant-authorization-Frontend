...
export const fetchPurchaseTaxes = createAsyncThunk<PurchaseTax[]>('purchaseTaxes/fetch', async () => {
  try {
    const response = await axios.get('http://192.168.29.116:8000/purchaseapi/purchasetaxes/');
...
export const addPurchaseTax = createAsyncThunk<PurchaseTax, Omit<PurchaseTax, 'purchasetaxId'>>(
  ...
  async (tax) => {
    try {
      const response = await axios.post('http://192.168.29.116:8000/purchaseapi/purchasetaxes', tax);
      return response.data;
    } catch (error: any) {
  ...
  async (tax, { rejectWithValue }) => {
    try {
      const response = await axios.patch(
        `http://192.168.29.116:8000/purchaseapi/purchasetaxes/${tax.purchasetaxId}`,
        tax
      );
      if (!response.data) {
...
export const deactivatePurchaseTax = createAsyncThunk<PurchaseTax, string>(
  ...
  async (id) => {
    try {
      const response = await axios.patch(`http://192.168.29.116:8000/purchaseapi/purchasetaxes/${id}`, { status: 'deactivated' });
      return response.data;
    } catch (error: any) {
...
export const activatePurchaseTax = createAsyncThunk<PurchaseTax, string>(
  ...
  async (id) => {
    try {
      const response = await axios.patch(`http://192.168.29.116:8000/purchaseapi/purchasetaxes/${id}`, { status: 'active' });
      return response.data;
    } catch (error: any) {
  ...
  async (file: File, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post(
        'http://192.168.29.116:8000/purchaseapi/purchasetaxes/import-csv',
        formData,
        {
          headers: {
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get(
        'http://192.168.29.116:8000/purchaseapi/purchasetaxes/export-tax/export-csv',
        {
          responseType: 'blob',
        }
      );
