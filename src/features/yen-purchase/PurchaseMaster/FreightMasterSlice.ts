...
export const fetchFreightItems = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get<Freight[]>('http://192.168.29.116:8000/purchasetestapi/freights/');
...
export const addFreightItem = createAsyncThunk<Freight, Omit<Freight, 'freightId'>>(
  ...
  async (freightData, { rejectWithValue }) => {
    try {
      const response = await axios.post('http://192.168.29.116:8000/purchasetestapi/freights/', freightData);
      return response.data;
    } catch (error: any) {
  ...
  async (freightData, { rejectWithValue }) => {
    try {
      const response = await axios.patch(
        `http://192.168.29.116:8000/purchasetestapi/freights/${freightData.freightId}`,
        freightData
      );
      return response.data;
    } catch (error: any) {
...
export const deactivateFreightItem = createAsyncThunk<Freight, string>(
  ...
  async (freightId, { rejectWithValue }) => {
    try {
      const response = await axios.patch(`http://192.168.29.116:8000/purchasetestapi/freights/${freightId}`, {
        status: 'deactivated',
      });
      return response.data;
    } catch (error: any) {
...
export const importCSV = createAsyncThunk<ImportResult, File>(
  ...
  async (file: File, { rejectWithValue }) => {
    ...
    try {
      ...
      const response = await axios.post('http://192.168.29.116:8000/purchasetestapi/freights/import-csv', formData, {
...
export const exportCSV = createAsyncThunk('freightItems/exportCSV', async (_, { rejectWithValue }) => {
  try {
    const response = await axios.get('http://192.168.29.116:8000/purchasetestapi/freights/export-csv', {
      responseType: 'blob',
    });
