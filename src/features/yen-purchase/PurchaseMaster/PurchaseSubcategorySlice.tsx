...
export const addPurchaseSubcategory = createAsyncThunk<PurchaseSubcategory, PurchaseSubcategory>(
  ...
  async (purchaseSubcategory) => {
    try {
      const response = await axios.post('http://192.168.29.117:8000/purchaseapi/purchasesubcategories/', purchaseSubcategory);
      return response.data;
    } catch (error: any) {
  ...
  async ({ purchasesubcategoryId, purchasesubcategory }) => {
    try {
      const response = await axios.patch(
        `http://192.168.29.117:8000/purchaseapi/purchasesubcategories/${purchasesubcategoryId}`,
        purchasesubcategory
      );
      return response.data;
    } catch (error: any) {
  ...
  async (file: File, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post(
        'http://192.168.29.117:8000/purchaseapi/purchasesubcategories/import-csv',
        formData,
        {
          headers: {
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get(
        'http://192.168.29.117:8000/purchaseapi/purchasesubcategories/exportsubcategory/export-csv',
        {
          responseType: 'blob',
        }
      );
