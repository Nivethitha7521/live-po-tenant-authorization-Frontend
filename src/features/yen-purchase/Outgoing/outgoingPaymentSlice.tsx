  ...
  async (filters: {
    ...
    try {
      ...
      const response = await axios.get<VendorDetail[]>(
        `http://192.168.29.116:8000/purchaseapi/outgoingpayments/vendors/details?${params.toString()}`
      );
      return response.data;
    } catch (error) {
      throw error;
...
export const fetchGRN = createAsyncThunk('purchaseorder/fetch', async () => {
  const response = await axios.get<GRN[]>(`http://192.168.29.116:8000/purchaseapi/grns/`);
...
export const addOutgoing = createAsyncThunk<Outgoing, Omit<Outgoing, 'outgoingId'>>('outgoings/addOutgoing', async (outgoingData) => {
  const response = await axios.post('http://192.168.29.116:8000/purchaseapi/outgoingpayments/', outgoingData); // Adjust the API endpoint as needed
  return response.data;
});
...
export const updateOutgoing = createAsyncThunk<Outgoing, Outgoing>('outgoings/updateOutgoing', async (outgoingData) => {
  const response = await axios.patch(`http://192.168.29.116:8000/purchaseapi/outgoingpayments/${outgoingData.outgoingId}`, outgoingData); // Adjust the API endpoint as needed
  return response.data;
});
// Thunk
...
>(
  ...
  async (vendorName, { rejectWithValue }) => {
    try {
      const response = await axios.get(`http://192.168.29.116:8000/purchaseapi/debitnote/vendor/${encodeURIComponent(vendorName)}/active-debits`);
  ...
  async (vendorNames, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get(
        `http://192.168.29.116:8000/purchaseapi/debitnote/multiplevendors/active-debits?vendor_names=${encodeURIComponent(vendorNamesStr)}`
      );
...
export const addNewPayment = createAsyncThunk<Outgoing, PaymentDetails>(
  ...
  async (paymentData, { rejectWithValue }) => {
    ...
    try {
      ...
      const response = await axios.post('http://192.168.29.116:8000/purchaseapi/outgoingpayments/', outgoingWithDate);
...
export const addNewVendorPayment = createAsyncThunk(
  ...
  async (paymentData: any, { rejectWithValue }) => {
    ...
    try {
      const response = await axios.post('http://192.168.29.116:8000/purchaseapi/outgoingpayments/advance/', {
        ...paymentData,
...
export const fetchTaxDetails = createAsyncThunk<TaxDetail[], string>(
  ...
  async (outgoingId) => {
    const response = await axios.get<TaxDetail[]>(`http://192.168.29.116:8000/purchaseapi/outgoingpayments/${outgoingId}/tax-details`);
...
>(
  ...
  async (
    ...
    try {
      ...
      const response = await axios.get<Outgoing>(`http://192.168.29.116:8000/purchaseapi/outgoingpayments/${outgoingId}`);
      ...
      await axios.patch(`http://192.168.29.116:8000/purchaseapi/outgoingpayments/${outgoingId}`, updatedOutgoing);
