...
>(
  ...
  async ({ status, filterBy, fromDate, toDate, vendorName }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get(`https://yenerp.com/purchasetestapi/advancevendor/vendorwise/advance?${params}`);
      return {
...
export const fetchVendorDetails = createAsyncThunk<VendorDetail[], { status?: string }, { rejectValue: string }>(
  ...
  async ({ status }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get(`https://yenerp.com/purchasetestapi/advancevendor/vendors`, { params });
...
>(
  ...
  async (payment, { rejectWithValue }) => {
    try {
      const response = await axios.post('https://yenerp.com/purchasetestapi/advancevendor/advance', payment);
      return response.data;
    } catch (error: any) {
  ...
  async (vendorId: string, { rejectWithValue }) => {
    try {
      const response = await axios.get(
        `https://yenerp.com/purchasetestapi/advancevendor/vendor/${vendorId}/advance-payments`
      );
...
export const fetchActiveAdvancesVendorByName = createAsyncThunk(
  ...
  async (vendorName: string, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get(
        `https://yenerp.com/purchasetestapi/advancevendor/vendorname/${encodedVendorName}/advance-payments`
      );
      return response.data.data;
    } catch (error: any) {
...
>(
  ...
  async (vendorNames, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get(
        `https://yenerp.com/purchasetestapi/advancevendor/vendors/active-advances?vendorNames=${encodeURIComponent(vendorNamesStr)}`
      );
