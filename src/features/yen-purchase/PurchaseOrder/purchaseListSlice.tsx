...
export const revertApprovedToPending = createAsyncThunk(
  'purchaseOrders/revertApprovedToPending',
  async (purchaseOrderId: string, { rejectWithValue }) => {
    try {
      const response = await purchaseApi.patch(
        `/purchaseorders/revert-to-pending/${purchaseOrderId}`,
        {},
        {
          headers: {
            "Content-Type": "application/json",
      ...
      return rejectWithValue(
        error.response?.data || "Failed to revert purchase order to pending",
      );
    }
  },
);
