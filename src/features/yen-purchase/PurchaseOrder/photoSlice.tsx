...
export const fetchPhotosByOrderId = createAsyncThunk(
  ...
  async (orderId: string, { rejectWithValue }) => {
    try {
      const response = await axios.get(`http://192.168.29.117:8000/purchaseapi/photos/${orderId}`);
  ...
  async ({ orderId, files }: { orderId: string; files: File[] }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post(
        `http://192.168.29.117:8000/purchaseapi/photos/upload/${orderId}`,
        formData,
        {
          headers: {
  ...
  async ({ orderId, index, file }: { orderId: string; index: number; file: File }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.patch(
        `http://192.168.29.117:8000/purchaseapi/photos/edit/${orderId}/${index}`,
        formData,
        {
          headers: {
            'Content-Type': 'multipart/form-data',
...
export const deletePhoto = createAsyncThunk(
  ...
  async ({ orderId, index }: { orderId: string; index: number }, { rejectWithValue }) => {
    try {
      await axios.delete(`http://192.168.29.117:8000/purchaseapi/photos/${orderId}/${index}`);
